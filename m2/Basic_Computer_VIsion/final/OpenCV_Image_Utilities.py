...
def overlay_uchar_image(base, map, color, color3, out):
    ...
    if (base.ndim == 2):
        ...
        nrows, ncols, nchannels = np.shape(base) #(720,1280,3)

    
    # Get output image �umber of channels
    if (out.ndim == 2):
        nchannels_out = 1
    else:
        nchannels_out = np.shape(out)[2] # nchannels_out = 3
